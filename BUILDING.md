...
dj-yosof is a **cgo** program: the audio path links/compiles C code (Opus via
`layeh.com/gopus`, Ogg/Vorbis via `github.com/xlab/vorbis-go`). So a build host
needs a C toolchain and a few `-dev` libraries in addition to Go.

Dependencies are **vendored** (committed under `vendor/`), so no module
downloads are required at build time — only the system C libraries below.
